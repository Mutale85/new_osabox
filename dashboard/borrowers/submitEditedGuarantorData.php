<?php
	...
	if (isset($_POST['firstname'])) {
		...
		$sql = $connect->prepare("UPDATE guarantors SET branch_id = ?, firstname = ?, lastname = ?, business = ?, gender = ?, ID = ?, country = ?, city = ?, address = ?, email = ?, phone = ?, dateofbirth = ?, working_status = ?, photo = ?, files = ?, loan_officers = ? WHERE id = ?  AND parent_id = ? ");
		$ex = $sql->execute(array($branch_id, $firstname, $lastname, $business, $gender, $ID, $country, $city, $address, $email, $phone, $dateofbirth, $working_status, $photo, $files, $officers, $user_id, $parent_id));
		if($ex){
			echo "done";
		}else{
