<?php
	...
	function getBorrowerFullNamesByCardId($connect, $borrower_ID) {
		$query = $connect->prepare("SELECT * FROM borrowers_files WHERE id = ? AND parent_id = ? ");
		$query->execute(array($borrower_ID, $_SESSION['parent_id']));
		$output = "";
		$row = $query->fetch();
		if($row){
		...
		if($row){
			...
			<?php
				...
				foreach ($file as $doc) {?>
	              	...
	              	<a href="fileuploads/<?php echo $doc?>" download class="float-right" title="download <?php echo $doc?>">
	              		<?php echo $doc?> </i>	
	              	</a>
	              	<br>
			<?php
	...
	function borrowerDetails($connect, $id) {
		...
		foreach ($res as $row) {?>
			<tr>
				...
				<td> <!-- Loan Status will be called from the db kukula --> </td>
				<td>
	...
	function currentLoan($connect, $borrower_id){
		$output = '';
		$query = $connect->prepare("SELECT * FROM `loans_table` WHERE id = ? ");
...
// =============== end of recept function =========================
	...
	function countLoans($connect, $borrower_id, $parent_id) {
		$output = '';
		$query = $connect->prepare("SELECT * FROM `loans_table` WHERE id = ? AND parent_id = ?");
		$query->execute(array($borrower_id, $parent_id));
		$output = $query->rowCount();
		return $output;
	}
