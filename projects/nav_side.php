...
<nav class="main-header navbar navbar-expand navbar-white navbar-light fixed-top">
    ...
    <ul class="navbar-nav ml-auto">
      ...
      <?php 
        $query = $connect->prepare("SELECT * FROM projects_team_members WHERE parent_id = ? AND branch_id = ? AND team_member = ? AND seen = '0' ORDER BY tid DESC ");
        $query->execute(array($_SESSION['parent_id'], $BRANCHID, $_SESSION['user_id']));
        $count = $query->rowCount();
        if($count > 0){
          $row = $query->fetch();
          if($row){
            $time = '<span class="float-right text-muted text-sm">'.time_ago_check($row['date_added']).'</span>';
          }else{
            $time = '<span class="float-right text-muted text-sm"></span>';
          }
        }else{
          $time = '<span class="float-right text-muted text-sm"></span>';
        }

        $sql = $connect->prepare("SELECT * FROM `todolist` WHERE parent_id = ? AND branch_id = ? ");
        $sql->execute(array($_SESSION['parent_id'], $BRANCHID));
        $count_all = $sql->rowCount();
      ?>
      <li class="nav-item dropdown">
        ...
        <div class="dropdown-menu dropdown-menu-lg dropdown-menu-right">
          <a href="project-asssigned?staff=<?php echo $_SESSION['user_id']?>" class="dropdown-item">
            <i class="bi bi-clipboard-check mr-2"></i><?php echo $count?> Projects
            <?php echo $time?>
          </a>
          <div class="dropdown-divider"></div>
          <a href="kanban" class="dropdown-item">
            <i class="bi bi-table mr-2"></i> <?php echo $count_all;?> Tasks Added
    ...
    <div class="sidebar">
      ...
      <nav class="mt-2">
       ...
       <?php
          ...
          <li class="nav-item">
            <a href="view-projects" class="nav-link text-primary">
              <i class="bi bi-binoculars nav-icon"></i>
              <p>Projects View</p>
            </a>
          </li>
          <li class="nav-item">
            <a href="projects-progress" class="nav-link text-info">
              <i class="bi bi-arrow-bar-right nav-icon"></i>
              <p>Projects in progress</p>
            </a>
          </li>
          <li class="nav-item">
            <a href="projects-on-hold" class="nav-link text-warning">
              <i class="bi bi-arrow-bar-right nav-icon"></i>
              <p>Projects Onhold</p>
            </a>
          </li>
          <li class="nav-item">
            <a href="projects-cancelled" class="nav-link text-danger">
              <i class="bi bi-arrow-bar-right nav-icon"></i>
          ...
          <li class="nav-item">
            <a href="projects-completed" class="nav-link text-success">
              <i class="bi bi-arrow-bar-right nav-icon"></i>
          ...
          <?php if($_SESSION['user_role'] == 'Admin'):?>
            ...
            <li class="nav-header text-secondary border-bottom">Email or SMS Clients</li>
              <li class="nav-item">
                <a href="email-clients" class="nav-link  text-dark">
                  <i class="bi bi-mailbox nav-icon"></i>
                  <p>Email Clients</p>
                </a>
              </li>
              <li class="nav-item">
                <a href="sms-clients" class="nav-link text-dark">
                  <i class="bi bi-reply-all nav-icon"></i>
                  <p>SMS Clients</p>
                </a>
              </li>
            <li class="nav-header text-secondary border-bottom"></li>
            <li class="nav-item">
            <a href="navtree" class="nav-link">
