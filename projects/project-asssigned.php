...
<body class="hold-transition sidebar-mini layout-fixed">
      					...
      					<div class="col-md-12">
			      			<div class="card card-primary">
						        <div class="card-header">
						          	<h3 class="card-title"><img alt="<?php echo getStaffMemberImage($connect, $_GET['staff'], $_SESSION['parent_id'])?>" class="table-avatar img-circle" src="<?php echo getStaffMemberImage($connect, $_GET['staff'], $_SESSION['parent_id'])?>" style="width: 50px; height: 50px;"> Projects</h3>
						          	...
						          	<table class="table table-striped projects" id="projectsTable">
						              	...
						              	<tbody>
						              		<?php
						              			...
						              			$query = $connect->prepare("SELECT * FROM projects_team_members WHERE parent_id = ? AND team_member = ? ");
												$query->execute(array($parent_id, $staff));
												...
												$query->execute(array($parent_id, $BRANCHID));
												if ($query->rowCount() > 0) {
													foreach ($query->fetchAll() as $row) {
														...
														if($status == 'Starting'){
															...
															$status_ = '<span class="badge badge-success">'.$status.'</span>';
														}
											...
											<tr>
						                      	<td><?php echo $one++ ?></td>
						                      	<td>
						                          	<a href="<?php echo $tid?>" data-staff="<?php echo $_GET['staff']?>" data-id="<?php echo $id?>" class="createCookieView"><?php echo $project_name?></a>
						                          	<br/>
						                          	<small>
												...
												<td class="project-actions text-right">
													<a href="<?php echo $tid?>" data-staff="<?php echo $_GET['staff']?>" data-id="<?php echo $id?>" class="btn btn-primary btn-sm createCookieView">
	...
	<script>
		...
		$(document).on("click", ".createCookieView", function(e){
	      	...
	      	var cname = "projectID";
	      	var projectViewed = 'projectViewed';
	      	var teamMemberId = 'teamMemberId';
	      	projectID(cname, cvalue);
	      	projectID(projectViewed, staffId);
	      	projectID(teamMemberId, tid);
