<?php
	...
	foreach ($reciever_email as $email) {	
			...
			<!doctype html>
    			<html lang="en-US">
              	<head>
					...
					<title>Email</title>
					<meta name="description" content="Loan Email For '.getClientsNamesByEmail($connect, $email, $parent_id).'">
					<style type="text/css">
				...
				<body style="font-family:sans-serif; background-color: #f2f3f8;" marginheight="0" topmargin="0" marginwidth="0" leftmargin="0">
					...
					<div class="messageBody" style="background-color: #ffffff; max-width:670px; margin:0 auto;padding:25px;">
						<h1 class="title" style="text-align:center;margin-bottom:20px; font-size:1.2em;">Hello '.getClientsNamesByEmail($connect, $email, $parent_id).'</h1>        
		...
		try {
		    ...
		    $mail->addAddress($email, getClientsNamesByEmail($connect, $email, $parent_id));     //Add a recipient
