<?php
	...
	if (isset($_POST['fetchCompletedTasksKanBan'])) {
		$branch_id = $_POST['branch_id'];
		$sql = $connect->prepare("SELECT * FROM `todolist` WHERE parent_id = ? AND branch_id = ?  AND task_status = 'done' ORDER BY id DESC ");
	 	...
	 	if ($sql->rowCount() > 0) {
	 		...
	 		?>
	 		<div class='task border-success' draggable='true'>
				...
				<div class='task__stats'>
					<span><time datetime="2021-11-24T20:00:00"><i class="bi bi-clock-fill text-success"></i> <?php echo date("M-d, Y", strtotime($completed_date)) ?></time></span>
					<span><i class="bi bi-person"></i> <?php echo getStaffMemberNames($connect, $posted_by, $parent_id)?> </span>
