<?php
	...
	function getBorrowerFullNamesByPhone($connect, $phone) {
		$query = $connect->prepare("SELECT * FROM borrowers WHERE borrower_phone = ? AND parent_id = ? ");
	...
	function getBorrowerFullNamesByEmail($connect, $email) {
		$query = $connect->prepare("SELECT * FROM borrowers WHERE borrower_email = ? AND parent_id = ? ");
	...
	function getStaffMemberImage($connect, $user_id, $parent_id) {
		...
		if($row){
			extract($row);
			if ($photo == "") {
				$image_src = 'dist/img/user2-160x160.jpg';
			}else{
		...
		if($row){
			...
			<?php
				...
				foreach ($file as $doc) {?>
	              	...
	              	<a href="http://localhost/kukula/k/fileuploads/<?php echo $doc?>" download class="float-right" title="download <?php echo $doc?>">
	              		<?php echo $doc?> </i>	
	              	</a>
	              	<br>
			<?php
	...
	function getBorrowersGroupMembers($connect, $id) {
		...
		if ($row['borrowers_id'] != "") {
			...
			foreach ($member as $member_id) {
               	$output .= '<option value="'.$member_id.'" selected>'. getBorrowerFullNames($connect, $member_id, $_SESSION['parent_id']).'</option>';	
			}
		}
		return $output;
	}

	function borrowersMeemberID($connect) {
		$query = $connect->prepare("SELECT * FROM group_borrowers WHERE id = ? ");
		$query->execute(array($id));
		$output = "";
		$row = $query->fetch();
		if ($row['borrowers_id'] != "") {
			$member = explode(", ", $row['borrowers_id']);
			foreach ($member as $member_id) {
               	// $output .= '<option value="'.$member_id.'" selected>'. $member_id.'</option>';
               	$output .= $member_id;	
			}
		}
		return $output;
	...
	function getBorrowersGroupMembersByID($connect, $g_id, $branch_id, $group_unique_id, $parent_id) {
		...
		if ($row['borrowers_id'] != "") {
			...
			foreach ($member as $borrower_id) {
               	$output .= '<li>
               				'. getBorrowerFullNames($connect, $borrower_id, $_SESSION['parent_id']).'</li>';	
			}
			$output . "<ol>";
		}
		return $output;
	...
	function loanStatus($connect, $BRANCHID, $parent_id, $status) {
		$output = '';
		$sql = $connect->prepare("SELECT * FROM `loans` WHERE branch_id = ? AND parent_id = ? AND loan_status = ? ");
	...
	function currentLoan($connect, $borrower_id){
		$output = '';
		$query = $connect->prepare("SELECT * FROM `loans` WHERE borrower_id = ? ");
...
// ========== RECEIPT FUNCTIONS ================
	function getBorrowerAddress($connect, $borrower_ID, $parent_id){
		...
		if($row){
			...
			$output = '
				<address>
	                <strong>'.getBorrowerFullNamesByCardId($connect, $borrower_ID).'</strong><br>
	...
	function getTotalPaid($connect, $loan_number, $parent_id){
		$output = '';
		$sql = $connect->prepare("SELECT *, SUM(amount) AS total_paid FROM `loan_payments` WHERE  loan_number = ? AND parent_id = ? ");
	...
	function getTotalPrinciple($connect, $parent_id, $loan_number){
		$output = '';
...
// =============== end of recept function =========================
	...
	function countLoans($connect, $borrower_id, $parent_id) {
		$output = '';
		$query = $connect->prepare("SELECT * FROM `loans` WHERE borrower_id = ? AND parent_id = ?");
		$query->execute(array($borrower_id, $parent_id));
		$output = $query->rowCount();
		return $output;
	}
#============================= ORGARNISATION INFORMATION =================================================
	...
	function getOrganisationLogo($connect, $parent_id) {
		...
		if($query->rowCount() > 0){
			...
			if ($row) {
				$output = '
					<img src="members/adminphotos/'.$row['org_logo'].'" alt="'.$row['org_logo'].'" class="img-fluid img-responsive" width="80">
				';
			}
		}else{
	...
	function getOrganisationLogoDetailsForPDF($connect, $parent_id) {
		...
		if($query->rowCount() > 0){
			...
			if ($row) {
				$output = '
                    members/adminphotos/'.$row['org_logo'].'
				';
			}
		}else{
...
#============================= END OF ORGARNISATION INFORMATION =================================================
	...
	// ========= FRONT PAGEs ========================
	function countBorrowers ($connect, $parent_id) {
		$output = '';
	...
	function countAllLoans($connect, $parent_id){
		$query = $connect->prepare("SELECT * FROM loans WHERE parent_id = ?  ");
		$query->execute(array($_SESSION['parent_id']));
		$output = $query->rowCount();
		return $output;
	}
...
// project management system function =================================

	function countAllToDoTasks($connect, $parent_id, $branch_id){
		$query = $connect->prepare("SELECT * FROM `todolist` WHERE parent_id = ? AND branch_id = ? AND status = '0' ");
	...
	function countBranchProjects($connect, $parent_id, $branch_id){
		$query = $connect->prepare("SELECT * FROM `projects` WHERE parent_id = ? AND branch_id = ? ");
		$query->execute(array($parent_id, $branch_id));
		$count = $query->rowCount();
...
function TotalPaid($connect, $parent_id) {
    $output = '';
    $query = $connect->prepare("SELECT SUM(`net_pay`) AS total_paid FROM `payroll` WHERE parent_id = ?");
    $query->execute(array($parent_id));
    $row = $query->fetch();
    if ($row) {
        $output = $row['total_paid'];
    }
    return $output;
}

function StaffTotalPaid($connect, $staff_id, $parent_id) {
    $output = '';
    $query = $connect->prepare("SELECT SUM(`net_pay`) AS total_paid FROM `payroll` WHERE staff_id = ? AND parent_id = ?");
    $query->execute(array($staff_id, $parent_id));
    $row = $query->fetch();
    if ($row) {
        $output = $row['total_paid'];
...
function Generated_Payslips($connect, $parent_id){
	$query = $connect->prepare("SELECT * FROM payroll WHERE parent_id = ? ");
 	$query->execute(array($_SESSION['parent_id']));
 	return $query->rowCount();
}
function personal_Payslips($connect, $user_id, $parent_id){
