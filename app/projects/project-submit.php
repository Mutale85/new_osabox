<?php
	...
	if(!empty($_POST['project_id'])){
		...
		$update = $connect->prepare("UPDATE `projects` SET `project_name`= ?,`description`= ?,`status`= ?,`leader`= ?,`start_date`= ?,`deadline`= ?,`estimated_budget`= ?,`currency`= ?,`estimated_duration`= ?, `estimated_period`= ? WHERE id = ? AND parent_id = ? ");
		$ex = $update->execute(array($project_name, $description, $status, $leader, $start_date, $deadline, $estimated_budget, $currency, $estimated_duration, $estimated_period, $project_id, $parent_id));
		...
		$spent_budget = '0';
		$sql = $connect->prepare("INSERT INTO `projects`(`parent_id`, `project_name`, `description`, `status`, `leader`, `start_date`, `deadline`, `estimated_budget`, `currency`, `estimated_duration`, `estimated_period`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
		$ex = $sql->execute(array($parent_id, $project_name, $description, $status, $leader, $start_date, $deadline, $estimated_budget, $currency, $estimated_duration, $estimated_period));
